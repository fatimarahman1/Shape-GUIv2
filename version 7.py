...
def frameQuizPage():
    ...
    for i in range(questionAmount):
        randomInt = random.randint(0, len(list(dict.fromkeys(question_types)))-1)
...
def DisplayQuestion(randomQuestionTypeIndex):
    ...
    if questionList[randomQuestionTypeIndex[questionIndex]].questionType == 0:
    ...
    if questionList[randomQuestionTypeIndex[questionIndex]].questionType == 1:
        #Display the question and shape image
        labelQuestion = Label(frameQuiz, text=questionList[randomQuestionTypeIndex[questionIndex]].text)
        labelQuestion.grid(row=1, column=1)
        ...
        #radiobuttons with randomised answers
