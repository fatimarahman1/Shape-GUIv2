...
class Question:
    def __init__(self, text: str, questionType: int, shapeTrait: str):
        self.text = text
        self.questionType = questionType
        self.shapeTrait = shapeTrait
...
questionList = []
questionList.append(Question("What is the name of this shape?", 0, "name"))
questionList.append(Question("How many corners does this shape have??", 1, "corners"))
questionList.append(Question("What is the angle sum of this shape", 1, "angleSum"))
def frameHomePage():
    global frameHome
    global frameLesson
...
def frameQuizPage():
    ...
    for i in range(questionAmount):
        randomInt = random.randint(0, len(questionList) - 1)
        randomQuestion.append(randomInt)
...
def DisplayQuestion(randomQuestion):
    ...
    if questionList[randomQuestion[questionIndex]].questionType == 0:
        #Display the question and shape image
        labelQuestion = Label(frameQuiz, text=questionList[randomQuestion[questionIndex]].text)
        ...
        buttonSubmit = Button(frameQuiz, text="Submit", command=lambda:[SubmitAnswer(entryAnswer.get(), shapeList[randomShape[modifiedQuestionIndex]])])
        buttonSubmit.grid(row = 4, column = 1)

    if questionList[randomQuestion[questionIndex]].questionType == 1:
        #Display the question and shape image
        labelQuestion = Label(frameQuiz, text=questionList[randomQuestion[questionIndex]].text)
        labelQuestion.grid(row=1, column=1)
        ...
        #Multiple choice option for question


        #Submit button to check answer
        buttonSubmit = Button(frameQuiz, text="Submit", command=lambda: [
            SubmitAnswer(entryAnswer.get(), shapeList[randomShape[modifiedQuestionIndex]])])
        buttonSubmit.grid(row=4, column=1)
